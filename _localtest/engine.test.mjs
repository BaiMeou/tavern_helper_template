...
const baseData = () => ({
  晓光: {
    ...
    营养代谢: {
      今日摄入: { 卡路里: 1000, 蛋白质克: 18, 脂肪克: 22, 碳水克: 180 },
      体脂储备: 18,
      基础代谢率: 1450,
      蛋白质平衡: 0,
    },
    伤口: {},
    疲劳: { 肌肉疲劳: 35, 睡眠债务: 4, 累积负荷: '疲惫' },
...
async function main() {
  ...
  await T('非法推进值：写入 $前端操作 反馈', async () => {
    ...
    const r = h.fire({ stat_data: d });
    assert(
      typeof r.stat_data.$前端操作 === 'string' && r.stat_data.$前端操作.includes('无法识别'),
      `应反馈无法识别, 实际 ${r.stat_data.$前端操作}`,
    );
  });
  ...
  await T('灵力环境：丰沛灵脉次日回灵（+4环境+过夜休息）', async () => {
    ...
    d.$推进时段 = '次日';
    const r = h.fire({ stat_data: d });
    // 丰沛 +4/时段 + 过夜休息回灵(无庇护所=2×0.5床铺=1, 无火×1) ≈ +5
    assert(r.stat_data.晓光.狐类特性.灵力值 > 30, `丰沛次日应回灵, 实际 ${r.stat_data.晓光.狐类特性.灵力值}`);
  });

  await T('灵力环境：枯竭灵脉黄昏掉灵（-6/时段）', async () => {
    const h = await boot();
    const d = baseData();
    d.世界.地形 = { 灵脉强度: '枯竭' };
    d.晓光.狐类特性.灵力值 = 30;
  ...
  await T('灵力环境：未知灵脉强度按正常处理（不惩罚AI自由描述）', async () => {
    ...
    assert(
      r.stat_data.晓光.狐类特性.灵力值 === 30,
      `未知灵脉强度应按正常(不增不减), 实际 ${r.stat_data.晓光.狐类特性.灵力值}`,
    );
  });
